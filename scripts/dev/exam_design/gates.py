...
ULP_FACTOR = 32  # the Burgers development rule's initial-condition resolution, 32 float32 ulp

PASS, FAIL, NOT_APPLICABLE = "PASS", "FAIL", "NOT_APPLICABLE"
SCORABLE, GATE_FAILED, REFERENCE_INVALID, FAILED_INFRA = "SCORABLE", "GATE_FAILED", "REFERENCE_INVALID", "FAILED_INFRA"
...
GATES = (
    ...
    Gate("voltage_ceiling", "max_t V_hat(t) <= V_max + tau_vmax",
         ("voltage_v",), "all cases (the cycler holds V <= 4.2 V; this is protocol control, not a safety limit)",
         "max(2 x max reference overshoot of V_max, 32 float32 ulp at 4.2 V)"),
    Gate("voltage_floor", "min_t V_hat(t) >= V_min - tau_vmin",
         ("voltage_v",), "all cases (discharge stops at 2.5 V)",
         "max(2 x max reference undershoot of V_min, 32 float32 ulp at 2.5 V)"),
    Gate("capacity_bound", "0 < Q_hat_k <= Q_bound for every checkpoint k",
         ("capacity_ah",), "all cases",
         "Q_bound = min(negative, positive, lithium-inventory capacity) from the parameter set; no tolerance"),
    Gate("paired_repeat", "|y_hat(x) - y_hat(x')| <= 32 float32 ulp x max(1, |y|) when x' duplicates x",
         ("voltage_v", "temperature_c", "plating_margin_v", "capacity_ah"),
         "cases carrying a hidden duplicate in the same batch", "32 float32 ulp"),
)
...
class Tolerances:
    ...
    def as_dict(self) -> dict:
        return {k: getattr(self, k) for k in ("tau_v0", "tau_t0", "tau_vmax", "tau_vmin", "q_bound", "v_max", "v_min")} | {
            "evidence": self.evidence}


def calibrate(references: list[dict], ocv: dict[str, float], q_bound: float, v_max=4.2, v_min=2.5) -> Tolerances:
    ...
    """
    d_v0 = max(abs(r["outputs"]["voltage_v"][0] - ocv[r["case_id"]]) for r in references)
    d_t0 = max(abs(r["outputs"]["temperature_c"][0] - r["inputs"]["t_amb_c"]) for r in references)
    over = max(max(r["outputs"]["voltage_v"]) - v_max for r in references)
    under = max(v_min - min(r["outputs"]["voltage_v"]) for r in references)
    ulp = lambda x: ULP_FACTOR * F32_EPS * x  # noqa: E731
    return Tolerances(
        ...
        v_max=v_max,
        v_min=v_min,
        evidence={"max_ref_v0_minus_ocv": d_v0, "max_ref_t0_minus_ambient": d_t0,
                  "max_ref_overshoot_v": over, "max_ref_undershoot_v": under, "n_references": len(references)},
    )
...
def evaluate_case(pred: dict | None, reference: dict, case_inputs: dict, ocv: float, tol: Tolerances,
                  shapes: dict, twin_pred: dict | None = None, infra_failed: bool = False) -> dict:
    ...
    g["initial_temperature"] = PASS if abs(t[0] - case_inputs["t_amb_c"]) <= tol.tau_t0 else FAIL
    ...
    if twin_pred is None:
        ...
        if ok:
            for k in shapes:
                ...
                if np.any(np.abs(a - b) > ULP_FACTOR * F32_EPS * np.maximum(1.0, np.abs(a))):
                    ok = False

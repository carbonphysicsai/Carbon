    ...
    return [
        {
            ...
            **{k: float(np.round(cols[k][i], 4)) for k in b},
        }
        for i in range(n)
    ]
...
def uniform_cases(n: int, role: str, prefix: str) -> list[dict]:
    ...
    return [
        {
            "case_id": f"{prefix}-{i:04d}",
            **{k: float(np.round(rng.uniform(lo, hi), 4)) for k, (lo, hi) in b.items()},
        }
        for i in range(n)
    ]
...
def pilot_plan(n_cases=12, n_cycles=40, n_refine=4, timeout_s=1500) -> dict:
    ...
    checkpoints = list(
        range(1, n_cycles + 1)
    )  # every cycle, so any shorter horizon can be evaluated
    jobs = [
        {"case": c, "n_cycles": n_cycles, "checkpoints": checkpoints, "role": "pilot"}
        for c in cases
    ]
    ...
    jobs = [
        ...
        for c in cases
        if c["case_id"] in pick
    ] + jobs
    return {
        "plan": "battery-pilot-v1",
        "campaign_root": CAMPAIGN_ROOT,
        "spec": SPEC,
        "timeout_s": timeout_s,
        "jobs": jobs,
        "refined_case_ids": pick,
    }
...
if __name__ == "__main__":  # pragma: no cover
    ...
    Path(sys.argv[1]).write_text(json.dumps(pilot_plan(), indent=1))
...
def photonic_lhs(n: int, role: str, prefix: str) -> list[dict]:
    ...
    cols = {
        k: lo + (rng.permutation(n) + rng.uniform(size=n)) / n * (hi - lo)
        for k, (lo, hi) in b.items()
    }
    return [
        {
            "case_id": f"{prefix}-{i:04d}",
            **{k: float(np.round(cols[k][i], 3)) for k in b},
        }
        for i in range(n)
    ]
...
def pilot2_plan() -> dict:
    ...
    bj += [
        {"case": by_id[c], "n_cycles": 40, "checkpoints": ck, "role": "pilot"}
        for c in refine + ["pilot-0007"]
    ]
    ...
    pj += [
        {"case": c, "role": "pilot", "refined": True, "timeout_s": 2700} for c in first
    ]
    ...
    return {
        ...
        "children": [
            ...
            },
            {
                "phase": "photonic_refs",
                "overlay": "photonic",
                "config": {"jobs": pj, "timeout_s": 1200},
            },
        ],
        "battery_refined_case_ids": refine,
        "photonic_refined_case_ids": pick,
    }
...
TWIN_SLOTS = (
    (10, 0),
    (20, 1),
)  # position of a hidden duplicate -> position it duplicates (inside every nested prefix)
    ...
    for b in range(SCREEN_BATCHES):
        ...
        for pos, src in TWIN_SLOTS:
            slots.insert(
                ...
                dict(
                    slots[src],
                    case_id=f"{role}-dup{src}",
                    duplicate_of=slots[src]["case_id"],
                ),
            )
        out.append(slots)
    return out
...
def refs_plan_b() -> dict:
    ...
    for b in range(SCREEN_BATCHES):
        ...
        jobs += [
            {
                "case": {k: v for k, v in c.items() if k != "duplicate_of"},
                "n_cycles": MAIN_HORIZON,
                "checkpoints": MAIN_CHECKPOINTS,
                "role": r,
                "batch": b,
            ...
            cases.append(
                {
                    "case_id": c["case_id"],
                    "role": r,
                    **{k: c[k] for k in ("c1", "c2", "t_amb_c", "soc0")},
                }
            )
    return {
        "schema": "carbon.exam-design.inputs.v1",
        "campaign_root": CAMPAIGN_ROOT,
        "cases": cases,
    }

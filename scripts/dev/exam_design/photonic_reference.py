...
def _centerline(x: np.ndarray, y_near: float, y_far: float, x0: float, bl: float, br: float, length: float):
...
def geometry(gap_nm: float, length_um: float, mirror: bool = False) -> dict:
    ...
    for sign in (+1, -1):
        ...
        verts = np.concatenate([np.column_stack([x, top]), np.column_stack([x[::-1], bot[::-1]])])
        polys.append(verts)
    return {"lx": lx, "ly": ly, "lz": lz, "y_far": y_far, "polys": polys}


def solve_case(case: dict, refined: bool = False, wavelengths=None, progress=None) -> dict:
    import fdtdx
    import jax
    ...
    port = {"p1": (x_port, y_up), "p2": (x_port, y_dn), "p3": (g["lx"] - x_port, y_up), "p4": (g["lx"] - x_port, y_dn)}
        ...
        for wl in wls:
            for src in ("p1", "p3"):
                ...
                for i, v in enumerate(geo["polys"]):
                    ...
                    poly = fdtdx.ExtrudedPolygon(materials=mats, material_name="si", axis=2, vertices=vv,
                                                 partial_real_shape=(None, None, h), name=f"wg{i}")
                    polys.append((poly, (cx, cy, cz)))
                inp = [fdtdx.PortSpec(center=(*port["p1"], cz), axis=0, direction="+", width=pw, height=ph, name="src")]
                outs = [fdtdx.PortSpec(center=(*port[p], cz), axis=0, direction=out_dir[p], width=pw, height=ph,
                                       name=f"d_{p}") for p in ("p2", "p3", "p4")]
                ts = time.perf_counter()
                objects, arrays, config = fdtdx.setup_sparams_simulation(
                    polygons=polys, input_ports=inp, output_ports=outs, wavelength=wl * 1e-6, resolution=res,
                    max_time=t_sim, domain_size=(g["lx"], g["ly"], g["lz"]), background_material=mats["cladding"],
                    pml_layers=SPEC["pml_layers"])
                tset = time.perf_counter()
                sp, _ = fdtdx.calculate_sparam(objects, arrays, config, input_port_name="src", show_progress=False)
                sp = {k: complex(np.asarray(jax.device_get(v)).reshape(-1)[0]) for k, v in sp.items()}
                ...
                rec["runs"].append({"wl_um": wl, "src": src, "setup_s": tset - ts, "run_s": te - tset})
                if progress:
                    progress(rec)
...
def checks(S: dict, wls) -> dict:
    ...
    for wl in map(str, wls):
        c = lambda k: complex(*S[k][wl]) if k in S and wl in S[k] else None  # noqa: E731
        ...
        out[wl] = {
            ...
            "reciprocity_31_13": abs(c("p3<-p1") - c("p1<-p3")) if c("p3<-p1") is not None and c("p1<-p3") is not None else None,

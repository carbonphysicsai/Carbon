# SPEC.md — Julia/SciML Ground Truth Oracle (Minimal Viable)
...
// Response
{
  "solution": [[...]],      // Flattened solution array
  "coords": [[...]],        // Grid coordinates
  "times": [0.0, 0.01, ...], // For time-dependent
  "metadata": {
    "solver": "Vern9",
    "abstol": 1e-12,
    "reltol": 1e-12,
...
// Request
{
  ...
  "initial_state": [[...]],     // Flattened initial state
  "params": { "mach": 0.8, ... },
  "loss_function": "physics_residual"  // or "combined_loss"
}

// Response
{
  "adjoint_gradients": [[...]],  // Exact adjoint gradients
  "rel_error": 1.2e-12,          // vs finite-difference
...
// Response
{
  "jax_code": "def loss_fn(params, state):\n    term1 = lambda1 * (div(u))**2\n    term2 = lambda2 * (drho_dt + div(rho*u))**2\n    return term1 + term2",
...
// Request
{
  ...
  "model_prediction": [[...]],  // Model output on reference grid
  "pde_spec": { ... },          // Same as /solve_pde
  "params": { ... }
}
...
function compute_adjoint_sensitivity(request::Dict)
    # SciMLSensitivity.jl for exact adjoint
...
function validate_against_reference(request::Dict)
    # Validate model prediction against SciML reference
...
function compute_adjoint_sensitivity(request::Dict)
    # Minimal adjoint for Phase 1A Adjoint Consistency Gate
    u0 = request["initial_state"]
    params = request["params"]
    ...
    # Adjoint via ReverseDiffVJP (exact, fast)
    adj_sol = adjoint_sensitivities(sol, loss_fn, 
        alg=InterpolatingAdjoint(autojacvec=ReverseDiffVJP()))
    
    # Compute relative error vs finite-difference
...
class MockSciMLClient:
    ...
    async def compute_adjoint_sensitivity(self, initial_state, params, loss_fn):
        # Analytic adjoint for linear PDEs
...
metadata:
  ...
  namespace: carbon
spec:
  replicas: 1  # Single replica for Phase 1A
  selector:
    matchLabels:
      app: carbon-sciml
  template:
    ...
    spec:
      ...
      - name: sciml-service
        ...
        - name: MOCK_FALLBACK
          value: "true"  # Fall back to mock on failure
        resources:
          requests:
            nvidia.com/gpu: 1
            memory: "32Gi"
...
spec:
  ...
  resources:
    requests:
      storage: 50Gi  # Smaller for Phase 1A
...
class SciMLValidationMixin:
    ...
    async def _run_adjoint_gate(self, state: TrainState) -> GateResult:
        try:
            ...
            # Mock fallback: compute finite-difference locally

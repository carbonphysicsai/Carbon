...
def main() -> None:
    try:
        ...
        print(
            "SKIP: Playwright is unavailable; validate_hub.py still enforces static content."
        )
        return
    ...
    with sync_playwright() as playwright:
        ...
        if len(visible) < 12_000:
            raise SystemExit(
                f"FAIL: primary index rendered only {len(visible)} visible characters"
            )
        ...
        nojs_context = browser.new_context(
            java_script_enabled=False, viewport={"width": 1440, "height": 1000}
        )
        ...
        if len(nojs_visible) < 12_000:
            raise SystemExit(
                f"FAIL: no-script index rendered only {len(nojs_visible)} visible characters"
            )
        if visible != nojs_visible:
            raise SystemExit(
                "FAIL: primary index content differs with JavaScript disabled"
            )
        nojs_context.close()
        ...
        if INTERACTIVE.exists():
            ...
            if interactive_visible < 500:
                raise SystemExit(
                    "FAIL: optional interactive view rendered too little content"
                )
            if app_errors:
                raise SystemExit(
                    f"FAIL: optional interactive view browser errors: {app_errors}"
                )

...
`tools/render_hub.py` generates the Overview as part of the Hub's single
deterministic generation/check pass by calling `tools/render_newcomer.py`'s
shared `collect_outputs()`:
...
handoff to the existing complete `index.html` Hub. `index.html` links back to
`newcomer.html` so both surfaces are reachable from the same page/system.
`tools/render_newcomer.py` remains independently runnable for focused
newcomer-only iteration; it is not a second source of generation truth because
`render_hub.py` imports and reuses the same functions.
...
The Overview is generated and checked by the Hub's normal required commands
(no separate CI step is required):
...
`tools/render_newcomer.py` and `tools/render_newcomer.py --check` remain
available for focused iteration on newcomer copy alone.

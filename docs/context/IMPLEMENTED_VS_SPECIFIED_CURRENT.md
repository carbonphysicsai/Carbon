...
**Status:** OWNER-CANONICAL maturity ledger, reconciled through the B-07A
...
> snapshot ratified the exact local v2 research-service protocol. The B-07A
> shipping snapshot conditionally implements and tests only the shared v2 core,
> discovery resources, and local two-operation adapter after applicable
> acceptance and normal merge. B-07B is next, `todo`, and unstarted.
...
| Wave B local research-service protocol | Yes — B-07S exact v2 contract ratified | Partial — B-07A shared nominal core, exact discovery resources, historical store, and local discovery adapter only | No | `carbon_protocol_v1` remains unchanged; B-07A executes only `get_challenge_info` and `get_interaction_manifest`; the full dispatcher, ten downstream operations, task/prior/practice/resource providers, security/network/production qualification, and LIVE remain absent |

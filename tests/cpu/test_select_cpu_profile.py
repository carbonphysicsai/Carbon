...
def test_candidate_profile_preserves_scientific_owners_and_exact_authority_delta():
    ...
    assert not only_candidate_root_added(before, after.replace("cards", "other"))
    assert (
        select_cpu_profile(
            ["carbon/candidates/store.py", ".agent/CODE_AUTHORITY.toml"],
            transport_root_addition=True,
        )
        == "NETWORK_FOUNDATION"
    )
    assert (
        select_cpu_profile(["carbon/candidates/store.py", "carbon/fees/service.py"])
        == "RUNTIME_FULL"
    )
    assert {

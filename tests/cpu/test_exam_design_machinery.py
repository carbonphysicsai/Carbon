...
def _ref(cid: str, soc: float, t_amb: float, eta: float, t_max: float, rng) -> dict:
    ...
    return {
        "case_id": cid,
        "status": "OK",
        "inputs": {"c1": 1.0, "c2": 0.5, "t_amb_c": t_amb, "soc0": soc},
        "outputs": {
            "voltage_v": v.tolist(),
...
def store():
    ...
    for i in range(40):
        eta = (
            -0.003 if i % 5 == 0 else 0.05
        )  # every fifth case sits at the plating decision boundary
        refs[f"c{i:03d}"] = _ref(
            f"c{i:03d}", 0.1 + 0.01 * i, 5 + i * 0.8, eta, 30 + i * 0.5, rng
        )
    refs["c001"] = refs["c000"] | {
        "case_id": "c001"
    }  # hidden duplicate: same inputs, same reference
    refs["c-bad"] = {
        "case_id": "c-bad",
    ...
    return exam.CaseStore(
        refs, ocv, tol, scoring.scales_from_train(ok), SHAPES, twins={"c001": "c000"}
    )
...
@pytest.mark.parametrize(
    "kind,gate",
    [(k, g) for k, g in controls.FAULTS.items() if g and g != "paired_repeat"],
)
def test_each_authored_fault_fires_its_own_gate(store, kind, gate):
    ids = [c for c in _ids(store) if c != "c-bad"]
    bad = controls.fault(
        kind, controls.oracle(store.refs, ids), q_bound=store.tol.q_bound
    )
...
def test_nondeterminism_fires_paired_repeat_only_where_a_twin_exists(store):
    ...
    rows, _agg = exam.evaluate(bad, ids, store)
...
def test_reference_failure_and_infra_are_never_charged_to_the_model(store):
    ...
    assert states == {
        "c002": "SCORABLE",
        "c003": "FAILED_INFRA",
        "c-bad": "REFERENCE_INVALID",
    }
    assert (
        agg["eligible"]
        and agg["n_reference_invalid"] == 1
        and agg["n_failed_infra"] == 1
    )
...
def test_nested_batch_prefix_uses_the_same_cases(store):
    ...
    assert set(small.active_case_ids()) <= set(
        exam.ScreeningPool(batches=batches, rotate_after=99).active_case_ids()
    )
...
def test_train_changes_only_by_named_version(store):
    ...
    assert tv.versions["train-v1"] == ["a", "b"] and tv.versions["train-v2"] == [
        "a",
        "b",
        "c",
    ]
...
def _cmp(delta, n=200, imp_every=4, seed=1, margin=0.02):
    ...
    chal = {
        k: v + delta(i) + rng.normal(0, 0.01) for i, (k, v) in enumerate(inc.items())
    }
    imp = {k: i % imp_every == 0 for i, k in enumerate(inc)}
    return exam.final_compare(
        inc, chal, imp, exam.ComparisonRule(equivalence_margin=margin)
    )
...
def test_seed_mixing_attack_gain_shrinks_with_faster_rotation():
    ...
    slow = simulate.seed_mixing_attack(
        e1, e2, batch_size=50, rotate_after=10, submissions=30, reps=60
    )
    fast = simulate.seed_mixing_attack(
        e1, e2, batch_size=50, rotate_after=1, submissions=30, reps=60
    )
    assert (
        slow["mean_gain"] > fast["mean_gain"] > 0
        or fast["mean_gain"] <= 0.5 * slow["mean_gain"]
    )
...
def test_regional_regression_blocks_promotion_even_with_an_overall_gain():
    ...
    assert (
        r["outcome"] == exam.TRADE_OFF and not r["promotable"] and r["regional_block"]
    )
...
def test_screening_nomination_refuses_regional_regression():
    ...
    assert exam.nominate(better_but_regional, inc, 0.02) == (
        False,
        "important-region regression beyond the margin",
    )
...
def test_capacity_component_weights_fade_by_its_own_spread():
    ...
    for i, r in enumerate(
        refs
    ):  # real-like: cycle-1 capacity varies ~60 mAh, 30-cycle fade ~2 mAh
        q1 = 4.8 + 0.003 * i
        r["outputs"]["capacity_ah"] = [
            q1,
            q1 - 0.002 - 0.0001 * i,
            q1 - 0.004 - 0.0002 * i,
        ]
    ...
    pred["capacity_ah"] = [
        q + 0.002 for q in ref["outputs"]["capacity_ah"]
    ]  # Q1 off, fade exact
    c_level = scoring.case_components(pred, ref, sc)["capacity"]
    pred["capacity_ah"] = [ref["outputs"]["capacity_ah"][0]] + [
        q + 0.002 for q in ref["outputs"]["capacity_ah"][1:]
    ]

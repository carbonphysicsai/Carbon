...
def test_browser_selected_gpu_runtime_reaches_the_real_reader(tmp_path, monkeypatch):
    ...
    # `load_worker_request` returns (ref, plan, archive, seed, split); it does
    # not hand back the profile it verified. The original of this test unpacked
    # a third value named `loaded_profile`, which is the training archive, so
    # its profile assertions could never have passed. The profile is checked
    # below against the reader's own rebuild instead of by renaming the archive.
    _ref, loaded_plan, _archive, _seed, _split = load_worker_request(stage)
    ...
    # The profile the reader rebuilds from the staged bytes, which is what the
    # digest comparison inside `load_worker_request` has already accepted.
    rebuilt = _request_worker_profile(json.loads((stage / "request.json").read_bytes()))
    assert rebuilt.accelerator_authority == MINER_HOST_AUTHORITY
    assert rebuilt.accelerator_device_uuid == device.device_uuid
    assert rebuilt.accelerator_grant_digest == device.digest
    # Verification only: it carries no bound, so it cannot supply one.
    assert not hasattr(rebuilt, "effective_deadline_seconds")
    assert not hasattr(rebuilt, "effective_output_bytes")

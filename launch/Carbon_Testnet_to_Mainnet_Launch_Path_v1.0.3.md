...
> Wave-B language, and former G2–G7 taxonomy are retained as historical design
> evidence only. `OWNER-NET-01` prospectively supersedes them; they do not
> authorize current work, chain action, settlement, or launch.
...
**Repository baseline:** main @ ea7f78b455f14f8ea674c196db349fd08b355305

**Planning basis:** Recent Codex implementation pace. The Wave B estimate is conditional: one qualified primary lane implies 15-24 working weeks; two qualified, non-overlapping primary lanes imply 9-16 working weeks. Independent review, timely human decisions, and scientific/security/rights work remain additional constraints.
...
> [!CAUTION]
> **Wave B rebaseline:** [`.agent/WAVE_B.md`](../.agent/WAVE_B.md) version 0.3 replaces the former umbrella B-02/B-07 estimate with 24 bounded tickets. Its current conditional estimate is 72-120 serial primary-lane days or 45-76 elapsed engineering days with two qualified non-overlapping lanes, before scientific/security/rights delays. The v1.0.1 S1 and downstream calendar dates are therefore not current promises. Launch owners must approve staffing and issue a calendar rebaseline before testnet or mainnet dates are represented as current.
...
| **Wave B authoring/research substrate v0.3** | 72-120 serial; 45-76 elapsed with two qualified lanes | 81-138 | 15-36 SciML/protocol/security/rights days | High |
...
| **CTRL-04** | P0      | IN REVIEW  | Reconcile stale status, seed, reference, mock-fallback, operations and legacy-emission language. Candidate reconciliation is in this v1.0.3 package; exit still requires owner acceptance of the conflict ledger. | CDX / PL              | M          | CTRL-01          | S0         |
...
version 0.3 and its 24 ticket files. The former B-02 and B-07 umbrella rows are
...
Conditional total: **72-120 primary-lane days serial**, or **45-76 elapsed
...
| **C-02**   | P0      | TODO       | Real declarative JAX reconstruction backend, backend profile, and frozen-build reuse. Exit: complete producer-independent base reconstruction under pinned resources plus receipt support for registered repeat evidence. | CDX+VE+SCI / SCI      | L          | B-02B,B-02C,B-03,B-E1 | rebaseline |
...
| **C-07**   | P0      | TODO       | Validator orchestration across reconstruct-reference-measure-score-card under the registered ReconstructionEvidencePolicy. Exit: official identity preserved; no proxy/screen superiority; typed failures; no direct chain science. | CDX+VE / PL+SCI | L | C-01-06 | S2 |
...
| **D-06** | P0      | TODO       | Reconstruction × whole-case/trajectory campaign, stratified by stress regime, plus staged-evidence decision policy. Exit: dependence-aware coverage-calibrated interval, minimum resolvable improvement, indifference band, stopping/extension rule, and any futility false-elimination audit. | SCI+STAT / STAT | L | C-02,D-05,MQ-007/008 | S3 |
...
| **H-02** | P1      | TODO       | Promotion-grade repeated reconstruction, common fresh exam, and paired decision interval. Exit: no screen/proxy superiority; sequential evidence and dependence-aware SUPERIOR / NOT_SUPERIOR / INDETERMINATE tests, including the `INSUFFICIENT_EVIDENCE` reason. | CDX+STAT / SCI+STAT | L | H-01,D-06,MQ-009 | S4/S5 |

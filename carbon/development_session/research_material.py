...
def capabilities():
    ...
    return {
        ...
        "parameter_notes": {
            "width": "FNO and DeepONet: even, compatible with C-02 heads=2. transolver: divisible by heads. Actual memory/time admission applies",
            "n_modes": "FNO only, refused for other families; even only, since odd values alias in the installed FNO",
            "depth_remat": "FNO and transolver; refused for DeepONet",
            ...
            "heads": "transolver only; attention heads, and width must divide evenly across them",
            "slices": "transolver only; learned physical-state slices each head attends over (Transolver-style)",
            "expansion": "transolver only; width multiplier of each block's feed-forward layer",

...
The helpers in this module are deliberately private.  A7's service owns the
submission boundary; this module only detaches hostile values, preserves A2 as
the first semantic authority, and constructs the versioned identities used by
that service.
"""
...
def _primitive_payload_size(value: object, string_limit: int) -> tuple[int, bool]:
    ...
    if value_type is str:
        return _utf8_scan(value, string_limit)
    return 0, False
...
def _capture_strategy(
    ...
    while work:
        ...
        if source_type is dict or source_type is list:
            ...
            items = _stable_dict_items(source, cardinality)
            ...
            for source_key, child in items:
                if type(source_key) is not str:
                    ...
                    continue
                key_bytes, key_invalid = _utf8_scan(
                    source_key,
                    min(
...
def _binding_payloads_representable(challenge_key: ChallengeKey) -> bool:
    ...
    return all(
        type(field) is str and str.isascii(field) and str.__len__(field) <= _UINT32_MAX
        for field in fields
    )
...
def _validate_and_hash_strategy(
    ...
    *,
    ...
    try:
        captured = _capture_strategy(strategy, limits)
    ...
    if challenge_key is not None:
        ...
        if (
            ...
            or not _binding_payloads_representable(challenge_key)
        ):
            return _invalid_identity_result(
                validation=validation,
                value_nodes=captured.value_nodes,
    ...
    try:
        strategy_hash, identity_bytes = _strategy_hash(
            captured.candidate, limits.max_strategy_identity_bytes
        )

...
the same `SPEC`, simulation, solve and extraction. A test holds the two
specifications equal. The reference is *the specified model*, PyBaMM's DFN
under OKane2022, not a real cell. Agreement with it says nothing about
real-cell lifetime or safety.

PyBaMM is imported only inside the functions that need it. It is pinned in the
truth image's overlay lock (`TRUTH_IMAGE`), not in Carbon's own environment, so
importing this module never loads it.

Failures are typed so a reference problem is never scored as a candidate
failure: ``REFERENCE_SOLVER_FAILED``, ``REFERENCE_TIMEOUT`` and
``FAILED_INFRA`` (see `truth.py`). Only ``OK`` references enter an exam.
"""

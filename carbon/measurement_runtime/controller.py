...
from carbon.reconstruction.worker.protocol import decode_output_stream, snapshot_output
...
from .protocol import stage_measurement_request, validate_measurement_snapshot_bounded
...
class IsolatedMeasurementResult:
...
class IsolatedBurgersMeasurementController:
    """Execute one fixed measurement job and associate only after cleanup."""
    ...
    def execute(
        ...
        stage, stage_digest = stage_measurement_request(
            self.state_root / "staging", request, candidate, reference
        )
        ...
        container_name = "carbon-c05-" + launch_digest[7:39]
        ...
        if retained is not None:
            ...
            result = validate_measurement_snapshot_bounded(snapshot, stage)
            if (
                _snapshot_digest(snapshot) != retained["snapshot_digest"]
                or result.result_digest != retained["result_digest"]
            ):
                shutil.rmtree(stage, ignore_errors=True)
                raise WorkerFailure(WorkerCode.CONFLICT)
            shutil.rmtree(stage, ignore_errors=True)
            return IsolatedMeasurementResult(
                result,
                launch_digest,
                self.image.image_id,
        ...
        journal: dict[str, object] = {
            "schema": "carbon.c05.measurement-launch.v1",
        ...
        if not checked.eligible or checked.cpuset is None:
            journal.update({"state": "FAILED_INFRA", "terminal_code": checked.code})
            self._journal(launch_digest, journal)
            shutil.rmtree(stage, ignore_errors=True)
        ...
        try:
            ...
            if remaining <= 0:
                raise WorkerFailure(WorkerCode.DEADLINE)
            raw_parent = Path(
                tempfile.mkdtemp(prefix=".c05-raw-", dir=self.state_root / "outputs")
            )
...
__all__ = ["IsolatedBurgersMeasurementController", "IsolatedMeasurementResult"]

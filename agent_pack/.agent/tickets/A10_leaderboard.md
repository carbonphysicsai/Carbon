...
- [ ] Public fields only: hotkey (or anonymized id policy), score, challenge_id, challenge_version, timestamp, gate_pass summary
- [ ] No seeds, draw ids, fine margins, internal diagnostics, fee amounts as rank features
- [ ] Stub results with `emission_capable=False` excluded from “official” board (or board is clearly `fixture` only in Wave A)

...
| **B-E4** | Preserve optional/deferred misuse and aligned-cheating research involving manufactured evidence, structural component claims, proxy residuals, reference failure, and candidate-specific evidence depth. Completed reusable runner, accounting, and evidence infrastructure remain available for a later funded resumption. | Research-specific utility, diversity, shadow, calibration, and ratification thresholds are optional/non-blocking. Concrete no-MMS-only-LIVE, no component-label bypass, no candidate-specific evidence-depth, and no proxy-as-official-evidence correctness checks belong to B-GATE and the shipping domain contracts. | Deferred research; not passed, qualified, completed, or a delivery prerequisite. |

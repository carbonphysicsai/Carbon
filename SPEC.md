...
**Spec alignment note (July 2026):** Productization uses a **dual threshold** (lean validator exams vs job-shaped product battery at Specialist Bank graduation). Landscape is a **four-port knowledge flywheel**, not a teacher-distillation step. Canonical detail: `appendices/Landscape_Agent.md` (v1.1+), `appendices/Specialist_Bank.md` (v1.3+), `appendices/Use_Cases_by_Phase.md`.
...
Model Cards (lean verified)
        ...
        → Port B: eval routing (validators)  (GPU compounds)
...
*Implementation details: `IMPLEMENTATION.md`. Landscape / bank: appendices cited above.*
...
3. **Route**  
   ...
   - B: spend validator GPU where it matters  
...
Success metrics for the flywheel: post-gate progress, PB pass rates, commercial conversion — **not** guidance-API engagement. Landscape never overrides gates.
...
|-----------|--------|-------------|
| **Physics Fidelity** | 45% | Gate margins averaged |
| **Robustness** | 30% | Mean performance across stress variants |
| **Accuracy** | 25% | Normalized benchmark error |
...
Seed map, train≠eval separation, stress categories (≥95% coverage), and custom dataset validation remain as specified. Critical invariant: `stress_seed` unknown to miners until evaluation; validator generator config ignores miner eval params.

---

## 10. Landscape Agent (Knowledge Flywheel)

Canonical build guide: **`appendices/Landscape_Agent.md` (v1.1+)**.  
...
Strategy schemas v1.0 / v1.1 / v2.0 remain as previously defined (`backbone`, boolean loss enables, optional LoRA/custom_dataset/structured_losses, composite coupling). Product-battery and Model Card extensions are versioned in Specialist Bank / Landscape contracts.

---

## 18. Security Checklist (Launch)

fp32 gates, boolean loss masks, JIT grad clip, determinism lockfile, compile cache, validator queue, dual-egress audit, grounding-gate enforcement on commercial export path.
...
| Doc | Role |
|-----|------|
| `appendices/Landscape_Agent.md` | Four ports, flywheel, build order |
...
*This specification is scientifically rigorous and buildable. Lean exams keep search and emissions honest; the Landscape flywheel compounds private intelligence into better search and eval; the Specialist Bank ships only gauntlet-verified products. Implementation must not collapse those layers into teacher-checkpoint distillation or pay-to-compete.*

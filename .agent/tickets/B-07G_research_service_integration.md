...
**Wave:** B candidate
**Status:** done
**Completion boundary:** authoritative after applicable OWNER-DX-03 acceptance
...
- [x] Enforce the ratified canonicalization, request/result bounds, nominal types, error precedence, safe projections, lifecycle/idempotency behavior, and resource accounting at the service boundary.
- [x] Provide only the ratified local in-process/agent adapter. Open no listener, accept no credentials, charge no fees, and imply no remote or production service.
- [x] Add a complete operation matrix covering owner/delegate, authority ceiling, disclosure class, resource class, errors, and prohibited cross-namespace behavior.
- [x] Add conformance, namespace-collision, canonicalization, malformed/bounded
...
- [x] Demonstrate that replacing a domain dependency with a conforming fixture changes only that domain behavior and that the service contains no duplicate compiler, provider, practice, forecasting, task, or record implementation.

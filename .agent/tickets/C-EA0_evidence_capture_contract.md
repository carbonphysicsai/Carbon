...
**Scope and reuse:** Reuse source identities from A7/current orchestration, `Evaluation_Evidence_and_Validator_Audit.md`, B-05, B-E1/B-E2, and the exact research protocol. Specify `EvidenceCaptureProfile`, `ArchiveEntry`, `ArtifactManifest`, `EvidenceUseAssessment`, retries/interruptions/failure classes, executed/unexecuted coverage, dependence, selection, guidance exposure, custody zones, and acknowledgement. Do not create a second result owner or set scientific values.

**Interfaces/failure/limits:** Contract references submitted strategy, resolved plan, versioned physical/execution contracts, observations, predictions/references/checkpoints, and artifact availability. Missing required evidence blocks finalization without becoming physics failure. Capture, use, release, and commercial entitlement remain separate; local unsubmitted experiments are excluded.

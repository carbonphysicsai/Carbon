...
- [x] Consume B-07A's B-07S-ratified shared wire-visible `ResearchTask` and
      ...
      the wire contract.
- [x] Keep operational task state separate from scientific outcome and evidence class.
- [x] Implement `ExperimentRecord` with exact contract pins, independently computed plan diff, evidence class, execution identity, typed failure, resource observations, and aggregate outcome refs.
- [x] Bind evidence-role identity, source/reference policy identity,
...
- [x] Preserve schema space for a later epistemic-status system without
      inferring or publishing causal authority in Wave B. An absent or future
      status cannot default to `experimentally_supported`.
- [x] Implement the B-07S-ratified bounded miner-facing `ResearchReceipt` projection with no protected identifiers or private record aliases.
- [x] Define `STRUCTURAL_ONLY`, `STATIC_EXACT`, `CALIBRATED_RESOURCE_FORECAST`, and `PRACTICE_NON_AUTHORITATIVE` without any conversion to `OFFICIAL_EVIDENCE`.
- [x] Preserve infrastructure, strategy, reconstruction, generator, reference,
      measurement, and scientific-admissibility failures as separate categories.
- [x] Retain scientifically meaningful failures without treating infra failures as negative scientific evidence.
- [x] Preserve failed reference, MMS/mutation, generator-conformance, hybrid-
...
- [x] Add idempotency, lineage, cancellation, requester-binding/local-adapter isolation, mutation-isolation, retention-scope, failure-class, evidence-role-confusion, MMS-relabeling, epistemic-default, leakage, and installed-wheel tests; make no authentication or Sybil-resistance claim.

...
Integrate the supplied typed goal-driven authoring contracts, CLI and verified
Burgers Dynamics V1 package into Carbon's existing registry/generator/reference/
measurement/score owners. Preserve the package's exact scientific law, rational
weights, evidence-role labels and corrected point-query semantics. Do not import
the workbench scheduler or duplicate Carbon lifecycle owners.
...
C-EA0 consumes the concrete authored execution graph before archive code relies
on it. C-02 may consume only a typed adapter until the authorized actual JAX

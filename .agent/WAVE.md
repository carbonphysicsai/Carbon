...
| A4 | C6 | Seeding domains + leakage tests | in_progress | `agent/a4-seeding` starts from exact base `e13baf312b811e2fd6784856c56d851a15f153fd`; bounded implementation under `carbon/seeding/` with `tests/cpu/test_seeding.py` and `tests/cpu/test_no_leakage.py`; focused command passed 230 tests and complete default CPU command passed 622 tests; independent review/merge/ancestry/post-merge CI remain pending |

...
> **Current bounded Wave-A status:** A0–A8 are accepted bounded foundations. The exact A9 Wave-A control/disclosure contract is ratified; its separately authorized implementation candidate is `in_progress` from exact base `47a62b2397b4125bb608eb69bf0e3dc6360c519d` on branch `agent/a9-mcp-skeleton`. A10–A12 remain `todo`. No ticket is relabeled as scientifically, security, network, commercially, or production qualified, and later-wave frontier, treasury, business, generalized-construction, and Physics-Intelligence architecture must not be smuggled into Wave A.
...
| A9 | C9 | MCP tools: get_challenge_info, get_prior, get_mock_scaffold, dry_validate, estimate, submit, get_submission_result | in_progress | Exact bounded contract ratified; candidate implementation from `47a62b2397b4125bb608eb69bf0e3dc6360c519d` on `agent/a9-mcp-skeleton`; implementation/test evidence remains candidate-only until independent review and merge; no qualification claim |

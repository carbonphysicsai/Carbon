...
> **Current Wave-A closeout candidate:** A0–A7 remain accepted bounded foundations; A8 joins them only after this documentation-only closeout is independently reviewed, explicitly human-authorized, and merged. A8's status is proposed `done` by this candidate; A9 has not begun and becomes the next unimplemented Wave-A ticket only after that merge. A9–A12 remain `todo`, and no ticket is relabeled as scientifically, security, network, commercially, or production qualified. Later-wave frontier, treasury, business, generalized-construction, and Physics-Intelligence architecture must not be smuggled into Wave A.

...
approval. B-01, B-01E, B-02A, B-07R, and B-02B are authoritatively `done` in
their recorded bounded scopes. B-02C is the selected `in_progress` ticket. Working engineering decisions
...
| B-02C | ResearchResourcePolicy, resource classes, ceilings, reconstruction-stage receipt seams, enforcement, and receipts | in_progress | `.agent/evidence/wave_b/b-02c.md` (working-contract phase; runtime implementation begins only after exact contract merge and exact-main CI) | Codex + SRE | Protocol + SRE + security + operations + economics | B-02B, B-07R | MQ-008, MQ-015, MQ-017, MQ-024 | M | WB-2 |

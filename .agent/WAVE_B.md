...
**Status:** closed in bounded engineering scope. This file remains the immutable
Wave B history and no longer selects active work.
**Version:** 4.9
...
removed from B-GATE and all later delivery dependencies. B-GATE is `done` in
bounded engineering scope after its accepted PR #118 delivery; no paid
experiment, qualification, or launch authority follows.
**B-GATE closeout:** accepted head
`c510095b5501587cf93c04632a26b7c303d7d48b` passed GitHub Actions run
`34365282759`, including Canonical environment, Development Hub validation,
and `Merge gate`, then normally merged as
`ac050fd53ee18bf3788c167ee7b0736bbfd7a46b` with tree
`46b38440f13a8048db18bc480ddb6907396c9017`. `OWNER-WAVE-C0-NET1-01`
prospectively moves active authority to `.agent/WAVE_C.md` without changing
this bounded maturity or any retained B-E4/B-01G status.
...
Version 4.6 records that PR #114 normally merged its accepted development-runner
head `7d8bc87efae35d1fddec3a9922eb70ebdbfee575` as
`5b378805472155a40374c71b961f07e2e3c8a823`, tree
`73e78ceb1e6d6e40204d5d5621144a36fd08d996`. The current successor records the
owner's approval of one 40-slot, non-qualifying DEVELOPMENT campaign and one
principal's five role-specific acts without treating them as independent
qualification ratification. It freezes execution request v2, adds a fresh
authenticated-owner issuer, one request/bindings entitlement, journal-bound
claim and restart, project/retention/source/limit/expiry/revocation validation,
the live Responses CLI route, honest partial/unknown-billing reports, and
controlled HTTP regressions over the full Carbon lifecycle. The current
workspace had neither the registered owner login nor provider project/key, so
no capability was issued, no provider request was dispatched, and no charge was
incurred. The exact external authentication/configuration handoff is recorded;
no policy reapproval is requested. Calibration, shadow, attack, qualifying
execution, v4 approval, and B-GATE remain outside this DEVELOPMENT authority.
B-E4 stays `in_progress`; B-GATE stays `todo` and unstarted.
Version 4.7 records that PR #115 normally merged the DEVELOPMENT admission
candidate as `6ac9e8db33f5e7da565f9617080f3bc4ed6b9745`. The current correction
preserves that architecture and every campaign choice while replacing only its
mistaken development-approver assignment with the authenticated `fitz-lang6` /
GitHub user `317786409` account. `jbequ5` / `99085788` remains the broader
repository owner. Owner-decision record v2 and execution request v3 bind the
prospective source/artifacts; v1/v2 history remains unchanged. The refreshed
deterministic 40-slot artifact passes with zero inference and spend, and the
production authenticator verifies the corrected viewer. No one-use
authorization was issued or claimed and no provider request was dispatched.
All experiment settings and the `$14.42` cap are unchanged. Calibration,
shadow, attack, qualifying execution, v4 approval, and B-GATE remain outside
this DEVELOPMENT authority. B-E4 stays `in_progress`; B-GATE stays `todo` and
unstarted.
...
| B-GATE | Fixture integration, invariant proof, closeout report, and no-placeholder-LIVE audit | done | `.agent/evidence/wave_b/b-gate.md`; `.agent/WAVE_B_REPORT.md`; PR #118 accepted head `c510095b5501587cf93c04632a26b7c303d7d48b`, run `34365282759`, merge `ac050fd53ee18bf3788c167ee7b0736bbfd7a46b`, tree `46b38440f13a8048db18bc480ddb6907396c9017` | Codex | Tech lead + science + protocol + security + rights; OWNER-DX-03 makes these notification/qualification lanes, not mandatory delivery reviewers | B-01, B-01E, B-01F, B-01H, B-02A, B-02B, B-02C, B-03, B-04, B-05, B-06, B-07R, B-07S, B-07A, B-07B, B-07C, B-07D1, B-07D2, B-07D3, B-07E, B-07F, B-07G, B-E1, B-E2, B-E3; B-01G and B-E4 explicitly non-blocking | MQ-001 through MQ-008, MQ-015 through MQ-018, MQ-024 through MQ-026, MQ-045, MQ-051 | M | WB-5 |
...
Wave C was unauthorized under the B-GATE candidate. The prospective
`OWNER-WAVE-C0-NET1-01` transition now moves `.agent/WAVE.md` to Wave C/C0 and
selects NET-1 alone without changing Wave B's bounded closure.

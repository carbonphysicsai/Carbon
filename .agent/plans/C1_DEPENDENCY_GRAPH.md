# C1 real-vertical dependency graph after C-08 PR #167

**Decision:** `OWNER-C1-CONTRACTS-01`
**Status:** authoritative planning checkpoint after merge
**Primary Hub map_ref:** `WAVE-C`
**Implementation selection:** PR #167 accepted C-08's bounded authenticated
DEVELOPMENT composition; C-EA1's private-alpha preparation is selected
...
```text
C-01(done) ─┬─> C-AUTH1(done) ─> C-EA0(done) ─> C-EA1(synthetic done; alpha preparation selected)
...
| C-EA1 | `in_progress` for `carbon.alpha-evidence-archive.private.v1` preparation; synthetic profile done | ticket and C-EA1-D2 profile/config/preflight sufficient for preparation | C-EA0, synthetic runtime and alpha owner policy satisfied | actual recovery/security/deployment acceptance remain open | provider/project/region, custody principals, recoverable services/keys, cost and deployment authorization absent | selected preparation; no real ack | **yes, current slice only** |
...
DEVELOPMENT composition. Under `OWNER-C1-BURGERS-ALPHA-01`, C-EA1's separately
versioned private-alpha preparation is now selected without another selection
ceremony. C-EA2 remains blocked until that work can produce an eligible real

...
Start from the current `main` HEAD and verify it before editing. Read, in order: root `AGENTS.md`; `agent_pack/EXECUTION_PROTOCOL.md`; `.agent/WAVE.md`; `.agent/INVARIANTS.md`; `.agent/DECISIONS.md`; `.agent/ORIENTATION.md`; `.agent/tickets/A0_repo_layout.md`; `Design_Specs/Build_Out_Protocol_Extension.md`; `Design_Specs/Evaluation_Evidence_and_Validator_Audit.md`; `.agent/plans/evaluation_evidence_protocol_extension.md`; then the Build_Out v1.4 sections cited by the ticket. Treat `.agent/ORIENTATION.md` as an audit of commit `0eed4e92609b4f26bd095a90f8cba9b7376fbe09`, not as proof that later code has been audited. Inspect the current diff/tree for drift before acting.
...
4. Establish lowercase `carbon/` as the canonical importable package. Promote or wrap only the minimum audited code needed for A0 layout acceptance. Reserve clean `evaluation/`, `audit/`, `chain/`, and `qualification/` boundaries and narrow adapter seams so scientific/evaluation modules do not couple deeply to Bittensor SDK objects. Treat Burgers as the first promotion source, Julia as the v0 generator-verification path, and `neurons/` as optional reuse—not as scientific qualification.
...
A0's boundary is package structure and adapter seams only. Do not implement later receipt, audit, chain, qualification, or Bittensor behavior during A0.

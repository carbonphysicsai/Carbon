# Carbon Reference-Cost Heterogeneity Study v0.1
...
This study extends the prior regime-conditioning and stratum-allocation work.
...
I repeated the Cole–Hopf calculation on 4,096 cases at the lowest development grid that the prior regime study treated as adequate for its diagnostic criterion:

| Regime | Development grid | Median compute / case |
|---|---:|---:|
| easy | 32 | 11.32 µs |
| moderate | 64 | 14.54 µs |
| hard | 96 | 24.13 µs |

The hard reference therefore cost about **2.13×** the easy reference in this analytic fixture.

Those timings are a Burgers implementation proxy, not an industrial CFD claim.
...
- moderate = 3× easy, hard = 12× easy;
- moderate = 5× easy, hard = 30× easy.
...
## Main result: cost-aware mean optimization can starve the hard stratum
...
When hard cases became much more expensive, that behavior changed.

Under the synthetic **30× hard-case** scenario, the same target-proportional budget is 282 easy-case-equivalent reference units. The cost-aware Neyman allocation became 37/13/6: it preserved many easy cases but did not buy additional hard evidence. Its target-mean RMSE was about **0.0603%** and its hard-p95 RMSE about **1.874%**.
...
To make the cost effect concrete, I defined a development-only precision target equal to the earlier 20/20/20 allocation:

- target-mean RMSE ≈ **0.0411%**;
- hard-stratum p95 RMSE ≈ **1.266%**.
...
|---|---:|---|
| measured Burgers (~2.13× hard/easy) | **86.7** easy-case-equivalent units | 24 / 14 / 21 |
| synthetic 12× hard/easy | **293** | 32 / 15 / 18 |
| synthetic 30× hard/easy | **644** | 39 / 13 / 18 |

The required scientific precision stayed fixed. Only the cost of acquiring evidence changed.

Under the 30× scenario, the original target-proportional 60-case budget of 282 units could not meet both development precision targets under any tested integer allocation.

That is the key Challenge-feasibility result:
...
For the synthetic 30× hard-case scenario, the 39/13/18 development precision plan costs 644 easy-case-equivalent units per fresh cohort:
...
This arithmetic does **not** qualify reuse. It excludes security/custody, adaptive-feedback leakage, reference exhaustion, fresh-cohort requirements, and any client/solver rights constraints.
...
The profiler should treat reference evidence as a constrained multi-objective allocation problem.
...
- The 12× and 30× hard-cost scenarios are synthetic stress tests.

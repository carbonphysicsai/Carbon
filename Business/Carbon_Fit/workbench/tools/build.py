...
def build():
 style=(ROOT/'src/styles.css').read_text()+'\n'+(ROOT/'src/goal_styles.css').read_text();engine=(ROOT/'src/engine.js').read_text();app=(ROOT/'src/app.js').read_text();workflow=(ROOT/'src/workflow.js').read_text();goal_app=(ROOT/'src/goal_app.js').read_text()
 atlas=data(ROOT/'data/atlas.json');studies=data(ROOT/'data/studies.json');cpes=data(ROOT/'data/cpes_study_v1.json')
 scripts=' '.join("'"+digest(s)+"'" for s in [engine,app,workflow,goal_app,atlas,studies,cpes])
 ...
 for k,v in {'CSP':csp,'STYLE':style,'ATLAS':atlas,'STUDIES':studies,'CPES':cpes,'ENGINE':engine,'APP':app,'WORKFLOW':workflow,'GOAL_APP':goal_app}.items():html=html.replace('{{'+k+'}}',v)

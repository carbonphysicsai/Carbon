...
test("client content never reaches the disk in the clear, in the store or a copy of it", async () => {
  const f = fresh();
  const receipt = await f.store.accept(packageWith(SENTINEL + " confidential operating range"), "e1-001", as("receiver"));
...
test("an approved deletion destroys the key first, and every earlier copy becomes unreadable", async () => {
  const f = fresh();
  const doomed = await f.store.accept(packageWith(SENTINEL + " to be deleted", "archive-doomed"), "e1-002", as("receiver"));
  const kept = await f.store.accept(packageWith("a record that stays", "archive-kept"), "e1-003", as("receiver"));
...
test("a sealed record cannot be moved onto another record", async () => {
  const f = fresh();
  const a = await f.store.accept(packageWith("record a", "archive-a"), "e1-004", as("receiver"));
  const b = await f.store.accept(packageWith("record b", "archive-b"), "e1-005", as("receiver"));
...
test("a store written before E1 is sealed at rest, and says what it could not reach", async () => {
  const f = fresh();
  const receipt = await f.store.accept(packageWith(SENTINEL + " pre-E1 plaintext", "archive-legacy"), "e1-006", as("receiver"));

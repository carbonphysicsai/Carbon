...
const { RELEASE, enrolled, keyringFor, mailed, openStore, principalFor, scoping } = require("./staff_fixture.cjs");
...
test("accepted inquiry is durable before receipt and exact response-loss retry deduplicates", async () => {
  ...
  const first = await fixture.store.accept(raw, "retry-key-001", roles.receiver, scoping(), mailed());
  ...
  const second = await restarted.accept(raw, "retry-key-001", roles.receiver, scoping(), mailed());
...
test("idempotency conflict and unauthorized access reject without mutation", async () => {
  const fixture = temporaryStore(), raw = reviewedRaw();
  await fixture.store.accept(raw, "retry-key-002", roles.receiver, scoping(), mailed());
  const before = JSON.stringify(fixture.store.state);
  await assert.rejects(
    () => fixture.store.accept(raw + " ", "retry-key-002", roles.receiver, scoping(), mailed()),
    /conflict/,
  );
  assert.throws(
...
test("optimistic updates preserve revisions and reject concurrent overwrite", async () => {
  ...
  const receipt = await fixture.store.accept(reviewedRaw(), "retry-key-003", roles.receiver, scoping(), mailed());
...
test("notification failure preserves inquiry and pending outbox for recovery", async () => {
  const fixture = temporaryStore();
  const receipt = await fixture.store.accept(reviewedRaw(), "retry-key-004", roles.receiver, scoping(), mailed());
...
test("authorized deletion removes data and retains only a synthetic tombstone", async () => {
  const fixture = temporaryStore();
  const receipt = await fixture.store.accept(reviewedRaw(), "retry-key-005", roles.receiver, scoping(), mailed());
...
test("private export requires the export role and preserves reviewed source bytes", async () => {
  ...
  const receipt = await fixture.store.accept(raw, "retry-key-006", roles.receiver, scoping(), mailed());
  assert.throws(
...
test("team assessments are append-only and retain every superseded revision", async () => {
  const fixture = temporaryStore();
  const receipt = await fixture.store.accept(reviewedRaw(), "retry-key-007", roles.receiver, scoping(), mailed());
...
test("a v1 store migrates without inventing a history it never retained", async () => {
  const fixture = temporaryStore();
  const receipt = await fixture.store.accept(reviewedRaw(), "retry-key-008", roles.receiver, scoping(), mailed());
...
test("a native store whose retained history was edited is refused", async () => {
  const fixture = temporaryStore();
  await fixture.store.accept(reviewedRaw(), "retry-key-009", roles.receiver, scoping(), mailed());
...
test("the queued notification carries a minimal summary and no client content", async () => {
  const fixture = temporaryStore();
  const receipt = await fixture.store.accept(reviewedRaw(), "retry-key-010", roles.receiver, scoping(), mailed());
...
test("with no configured transport an attempt fails observably and never claims delivery", async () => {
  const fixture = temporaryStore();
  const receipt = await fixture.store.accept(reviewedRaw(), "retry-key-011", roles.receiver, scoping(), mailed());
  const eventId = "notify-" + receipt.inquiry_id;
...
test("a configured destination is recorded without opening any connection", async () => {
  ...
  const receipt = await store.accept(reviewedRaw(), "retry-key-012", roles.receiver, scoping(), mailed());
...
test("a storage failure returns no receipt and stores no partial inquiry", async () => {
  ...
  try {
    await assert.rejects(() =>
      fixture.store.accept(reviewedRaw(), "retry-key-013", roles.receiver, scoping(), mailed()),
    );
  } finally {
  ...
  const receipt = await fixture.store.accept(reviewedRaw(), "retry-key-013", roles.receiver, scoping(), mailed());
...
test("completing a notification keeps writes made during the await window", async () => {
  const fixture = temporaryStore();
  const receipt = await fixture.store.accept(reviewedRaw(), "retry-key-014", roles.receiver, scoping(), mailed());
...
test("a transient flush failure does not block every later write", async () => {
  const fixture = temporaryStore();
  await fixture.store.accept(reviewedRaw(), "retry-key-016", roles.receiver, scoping(), mailed());
...
test("a delivery in flight cannot resurrect an inquiry deleted during its await", async () => {
  const fixture = temporaryStore();
  const receipt = await fixture.store.accept(reviewedRaw(), "retry-key-017", roles.receiver, scoping(), mailed());
  const eventId = "notify-" + receipt.inquiry_id;
  let release;
  const opened = new Promise((resolve) => {
    release = resolve;
...
test("archiving retains the record and removes it from the working set", async () => {
  const fixture = temporaryStore();
  const receipt = await fixture.store.accept(reviewedRaw(), "retention-001", roles.receiver, scoping(), mailed());
...
test("a deletion exception is named, reasoned and bounded", async () => {
  const fixture = temporaryStore();
  const receipt = await fixture.store.accept(reviewedRaw(), "retention-002", roles.receiver, scoping(), mailed());
...
test("a tombstone states what the deletion did not reach", async () => {
  const fixture = temporaryStore();
  const receipt = await fixture.store.accept(reviewedRaw(), "retention-003", roles.receiver, scoping(), mailed());
  approveDeletion(fixture.store, receipt.inquiry_id);
  const tombstone = fixture.store.delete(receipt.inquiry_id, roles.steward);
...
test("a v2 store migrates to versioned retention without back-dating an archive", async () => {
  const fixture = temporaryStore();
  const receipt = await fixture.store.accept(reviewedRaw(), "retention-004", roles.receiver, scoping(), mailed());
...
test("every retention guarantee fails against a store built to violate it", async () => {
  for (const guarantee of RETENTION_GUARANTEES) {
    ...
    const receipt = await control.store.accept(reviewedRaw(), "control", roles.receiver, scoping(), mailed());
    await guarantee.check(control.store, receipt);

    const subject = temporaryStore();
    const other = await subject.store.accept(reviewedRaw(), "variant", roles.receiver, scoping(), mailed());
...
test("the migration check rejects a store that back-dates an archive", async () => {
  ...
  const fixture = temporaryStore();
  const receipt = await fixture.store.accept(reviewedRaw(), "retention-005", roles.receiver, scoping(), mailed());
...
test("the negative-control harness itself fails when nothing is violated", async () => {
  ...
  for (const guarantee of RETENTION_GUARANTEES) {
    const fixture = temporaryStore();
    const receipt = await fixture.store.accept(reviewedRaw(), "meta", roles.receiver, scoping(), mailed());
    await assert.rejects(
      assert.rejects(
...
test("every store endpoint refuses a caller that was never authenticated", async () => {
  const fixture = temporaryStore();
  const receipt = await fixture.store.accept(reviewedRaw(), "role-001", roles.receiver, scoping(), mailed());
  approveDeletion(fixture.store, receipt.inquiry_id);
...
test("a principal from another team is refused as if the inquiry did not exist", async () => {
  const fixture = temporaryStore();
  const receipt = await fixture.store.accept(reviewedRaw(), "role-002", roles.receiver, scoping(), mailed());
...
test("a migrated record has no owning team and is reachable by nobody", async () => {
  const fixture = temporaryStore();
  const receipt = await fixture.store.accept(reviewedRaw(), "role-003", roles.receiver, scoping(), mailed());
  // A v2 store predates E1, so it is plaintext: built from the in-memory state.
  const legacy = JSON.parse(JSON.stringify(fixture.store.state));
  legacy.schema_version = "carbon.private-team-intake.store.v2";
  delete legacy.exceptions;
...
test("a store interrupted before its rename restarts on the last good state", async () => {
  const fixture = temporaryStore();
  const receipt = await fixture.store.accept(reviewedRaw(), "recover-001", roles.receiver, scoping(), mailed());
  fixture.store.update(
...
test("a damaged store refuses to open rather than starting empty", async () => {
  const fixture = temporaryStore();
  const receipt = await fixture.store.accept(reviewedRaw(), "recover-002", roles.receiver, scoping(), mailed());
  ...
  assert.equal(
    (await reopened.accept(reviewedRaw(), "recover-002", roles.receiver, scoping(), mailed())).disposition,
    "DEDUPLICATED",
  );
});
...
test("a failed write leaves the previous committed state readable", async () => {
  const fixture = temporaryStore();
  const first = await fixture.store.accept(reviewedRaw(), "recover-003", roles.receiver, scoping(), mailed());
...
test("the retention lifecycle is append-only across an archive and restore cycle", async () => {
  const fixture = temporaryStore();
  const receipt = await fixture.store.accept(reviewedRaw(), "provenance-001", roles.receiver, scoping(), mailed());
...
test("a retention history with an entry removed is refused", async () => {
  const fixture = temporaryStore();
  const receipt = await fixture.store.accept(reviewedRaw(), "provenance-002", roles.receiver, scoping(), mailed());
  fixture.store.archive(receipt.inquiry_id, roles.steward);
  fixture.store.restore(receipt.inquiry_id, roles.steward);
  fixture.store.archive(receipt.inquiry_id, roles.steward);
...
test("an archived record cannot be revised until it is restored", async () => {
  const fixture = temporaryStore();
  const receipt = await fixture.store.accept(reviewedRaw(), "archive-update-001", roles.receiver, scoping(), mailed());
...
test("each endpoint admits exactly the roles it is supposed to", async () => {
  ...
  for (const [endpoint, permitted] of Object.entries(MATRIX)) {
    for (const name of everyone) {
      const fixture = temporaryStore();
      const receipt = await fixture.store.accept(reviewedRaw(), "matrix", roles.receiver, scoping(), mailed());
      approveDeletion(fixture.store, receipt.inquiry_id);
      const id = receipt.inquiry_id;
      const call = {
        accept: (p) => fixture.store.accept(reviewedRaw(), "matrix-2", p, scoping(), mailed()),
...
test("a write that would breach the ceiling is refused and changes nothing", async () => {
  const fixture = temporaryStore();
  const receipt = await fixture.store.accept(reviewedRaw(), "ceiling-001", roles.receiver, scoping(), mailed());
  ...
  await assert.rejects(
    () => tight.accept(reviewedRaw(true), "ceiling-002", roles.receiver, scoping(), mailed()),
...
test("the refusal says what to do, and the alternatives it rules out", async () => {
  const fixture = temporaryStore();
  await fixture.store.accept(reviewedRaw(), "ceiling-003", roles.receiver, scoping(), mailed());
  const tight = openStore(fixture.file, {
    writeCeilingBytes: fs.statSync(fixture.file).size + 32,
  });
  let message = "";
  try {
    await tight.accept(reviewedRaw(true), "ceiling-004", roles.receiver, scoping(), mailed());
  } catch (error) {
...
test("capacity reports headroom before the wall rather than at it", async () => {
  ...
  await fixture.store.accept(reviewedRaw(), "ceiling-005", roles.receiver, scoping(), mailed());
...
test("a second process cannot start on a store another process is writing", async () => {
  const fixture = temporaryStore();
  await fixture.store.accept(reviewedRaw(), "lock-001", roles.receiver, scoping(), mailed());
...
test("a lock left by a dead process does not wedge the next start", async () => {
  const fixture = temporaryStore();
  await fixture.store.accept(reviewedRaw(), "lock-002", roles.receiver, scoping(), mailed());

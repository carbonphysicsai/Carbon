...
class SourceTests(unittest.TestCase):
    ...
    def test_exact_atlas_source_hash(self):
        self.assertEqual(hashlib.sha256(self.source.read_bytes()).hexdigest(), self.atlas["source"]["sha256"])
    ...
    def test_all_64_source_opportunities_and_groups_survive(self):
        ...
        self.assertEqual([sum(o["group"] == g for o in self.atlas["opportunities"]) for g in "ABCD"], [13, 17, 15, 19])
    ...
    def test_source_hypotheses_never_become_qualification(self):
        self.assertTrue(all(not o["qualified_carbon_evidence"] and o["profile_status"] == "NOT_PROFILED" for o in self.atlas["opportunities"]))
        self.assertTrue(all(o["equations_to_register"] is None and o["boundary_conditions_to_register"] is None for o in self.atlas["opportunities"]))

    def test_historical_citations_and_reports_preserved(self):
        self.assertEqual([r["id"] for r in self.atlas["bibliography"]], [f"C{i}" for i in range(1, 12)] + [f"E{i}" for i in range(1, 42)])
        for record in json.loads((ROOT / "data/studies.json").read_text())["studies"]:
            self.assertEqual(hashlib.sha256((ROOT / "sources" / record["filename"]).read_bytes()).hexdigest(), record["sha256"])
    ...
    def test_seed_index_parity(self):
        ...
        self.assertEqual([x["id"] for x in index["records"]], [x["id"] for x in self.atlas["opportunities"]])
    ...
    def test_all_eight_pinned_evidence_members_verify(self):
        ...
        self.assertEqual(hashlib.sha256(index_path.read_bytes()).hexdigest(), cpes.INDEX_SHA256)
    ...
    def test_evidence_member_schemas_and_cross_consistency(self):
        ...
        for name, schema in expected.items():
            self.assertEqual(json.loads((base / name).read_text())["schema_version"], schema)
        rows = json.loads((base / "attack_dispositions_v2.json").read_text())
        self.assertEqual([r["attack_id"] for r in rows], [f"AT-{i:02d}" for i in range(1, 31)])
        traces = [json.loads(line) for line in (base / "attack_traces_v2.jsonl").read_text().splitlines()]
        self.assertEqual(len(traces), 12)
        self.assertTrue(all("probe" in trace and "evidence_layer" in trace for trace in traces))
    ...
    def test_public_study_keeps_layers_unknowns_and_authority_separate(self):
        ...
        self.assertEqual(self.study["attack_counts"], {"BLOCKED": 5, "REJECTED_BY_PERSISTENT_RESEARCH_PROTOTYPE": 8, "REJECTED_BY_REPRODUCED_ORIGINAL_MODEL": 17})
        self.assertEqual(self.study["blocked_attack_ids"], ["AT-09", "AT-16", "AT-19", "AT-22", "AT-30"])
        ...
        self.assertFalse(self.study["observations"]["cross_hardware_composition_permitted"])
        self.assertTrue(all(value is None for value in self.study["quantity_unknowns"].values()))

    def test_adaptive_negative_control_exact_values(self):
        a = self.study["adaptive_negative_control"]
        self.assertEqual((a["whole_cases"], a["queries"], a["recovered_bank_exactly"], a["reused_bank_accuracy"], a["independent_holdout_accuracy"]), (32, 33, True, 1.0, 0.46875))

    def test_build_is_deterministic_and_script_safe(self):
        subprocess.run(["/usr/bin/python3", str(ROOT / "tools/build.py")], check=True, capture_output=True)
    ...
    def test_current_schema_is_closed_v02(self):
        ...
        self.assertEqual(schema["properties"]["schema_version"]["const"], "carbon_workbench_workspace_v0.2")
        self.assertFalse(schema["additionalProperties"])
        self.assertEqual(schema["properties"]["drafts"]["items"]["properties"]["qualification_status"]["const"], "NOT_QUALIFIED_BY_THIS_TOOL")
    ...
    def test_manifest_and_bundle_are_complete_and_reproducible(self):
        ...
        for name, meta in manifest["files"].items():
            ...
            self.assertEqual(hashlib.sha256(path.read_bytes()).hexdigest(), meta["sha256"], name)
        with zipfile.ZipFile(archive_path) as bundle:
            names = bundle.namelist()
            self.assertIn("carbon_opportunity_workbench_v0_2/MANIFEST.json", names)
            self.assertIn("carbon_opportunity_workbench_v0_2/Carbon_Opportunity_Workbench.html", names)
            self.assertIn("carbon_opportunity_workbench_v0_2/evidence/cpes_reference_reuse_v2/evidence_index_v1.json", names)

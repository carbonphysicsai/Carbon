...
(async () => {
  ...
  const errors = [];
  try {
    const page = await browser.newPage({ viewport: { width: 1440, height: 1000 }, acceptDownloads: true }); page.setDefaultTimeout(30000); page.on("pageerror", (error) => errors.push(error.message)); page.on("dialog", (dialog) => dialog.accept());
    ...
    if (mode === "prepare") {
      ...
      await page.locator('[data-tab="jobs"]').click();
      await page.locator("#science-connect").click(); await page.locator("#science-adopt").click(); await page.locator("#science-run:not([disabled])").waitFor(); await page.locator("#science-run").click();
      ...
      await page.locator("#science-result:not([disabled])").waitFor(); await page.locator("#science-result").click(); await page.locator("#science-plot svg").waitFor();
      await save(page, "#science-save", outputFile);
      const bundle = JSON.parse(fs.readFileSync(outputFile)), result = bundle.response;
      assert.equal(result.status, "COMPLETE"); assert.equal(result.official_eligible, false); assert.equal(result.qualification, "NOT_QUALIFIED"); assert.equal(result.result.values.length, 13); assert(result.result.values.every((row) => row.length === 64 && row.every(Number.isFinite)));

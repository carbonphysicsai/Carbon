...
| A store cannot write a file it cannot read | one ceiling for writes, provably below the read limit; the constructor refuses a configuration where it is not | ceiling symmetry, refusal-changes-nothing, independently-openable-backup and capacity tests, with a specimen showing the old 10 MB wedge was real | Single-file store; rotation is the operator's step, not automatic |
...
|---|---|---|---|
| Guided conversation and form edit one brief | pilot designer shell/app; shared `src/intake.js` | Chrome mode-switch continuity, proposal accept/reject/undo; reviewed-package pure tests | Guidance proposes; client accepts; no route or science is inferred |
| Explicit AI disclosure and optional history | versioned notice, consent gate, unchecked conversation inclusion | Chrome pre-request consent and export-content assertions; closed-package validation | Local clear does not delete provider records; final public notice remains pending |
| Bounded pilot output and provenance | `carbon.client-intake.reviewed.v1`, pilot fields, field origins and assumptions | accepted-suggestion attribution, unknown/authority injection rejection, Workbench import | Draft pilot is client-reviewed intake, not scientific/commercial acceptance |
| Shared Ask Carbon adapter and $50 monthly ceiling | worker `PILOT_DESIGN`; shared Durable Object ledger | core/worker structured-output tests; general+pilot monthly and per-session ledger tests | No second allowance; activation remains fail closed |
| Representative pilot-design evaluation surface | `website/ask-carbon/eval/pilot-design.cases.public.json` | authored contract-suite coverage and mock browser conversation | No live model-quality or customer-usability evidence |
| Short local-only primary flow with explicit unknowns and quantities | `Carbon_Client_Intake_Preview.html`; `src/intake.js`; closed draft schema | intake pure tests; actual Chrome form/summary/download at desktop and narrow width | Contact is local-export only; no submission, telemetry, fit score, quote, or customer claim |

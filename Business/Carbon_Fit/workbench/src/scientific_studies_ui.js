(function (root) {
  ...
  function create({ getDesign, notify, download }) {
    ...
    function draw() {
      ...
      panel.innerHTML = `<div class="eyebrow">Public source study · DEVELOPMENT</div><h3>Executable physical study</h3><p>Check the draft, then study the exact public Burgers source case through the private scientific service. This is not numerical validation of customer design text. Private customer inputs and operating-envelope sweeps are unavailable in this profile.</p><p><strong>Scientific qualification: NOT_QUALIFIED.</strong> Draft assessment and rights remain unchanged. Source-owner registration and an existing service grant are required.</p><div class="actions"><button id="science-check" ${locked}>Check physical definitions</button><button id="science-connect" ${!enabled ? "disabled" : locked}>Connect private service</button><button id="science-adopt" ${!state.capabilities?.available ? "disabled" : locked}>Adopt public source inputs</button><button disabled>Explore operating envelope unavailable</button></div><p id="science-connection">${enabled ? state.capabilities ? "Source profile received; no study runs until requested." : "Private service build; connection requires your explicit action." : "Offline build: numerical execution unavailable. Open this Workbench on its operator-configured private service to run a study."}</p><div id="science-check-result" role="status"></div><details><summary>Exact physical inputs and array layout</summary><pre id="science-inputs"></pre><p>Periodic unforced Burgers; Fourier coefficients use modes 1–12. Output rows follow the 13 requested times; columns are 64 periodic points from x=0, endpoint excluded, C order, zero-based indexing. Dimensionless variables; no implicit unit conversion.</p></details><div class="actions"><button id="science-run" ${!r || !enabled || r.association === "STALE" ? "disabled" : locked}>Assess reference feasibility</button><button id="science-status" ${!r || !enabled ? "disabled" : locked}>Status</button><button id="science-result" ${!r || !enabled ? "disabled" : locked}>Load results</button><button id="science-cancel" ${!r || !enabled ? "disabled" : locked}>Cancel study</button><button id="science-save" ${!r ? "disabled" : locked}>Save study</button><button id="science-reopen" ${locked}>Reopen saved study</button><input id="science-file" type="file" accept=".json,application/json" hidden></div><p id="science-state" role="status">${state.busy ? "Request in flight; execution state may require reconciliation." : r ? esc(r.association + " / " + r.origin + " / " + (response?.status || "NOT_EXECUTED")) : "No study adopted."}</p><p>Saved studies are separate versioned files bound to this job and exact draft revision. Reopened data is unverified until reread through the private service. Cancellation is a request; resource release must be observed by the controller.</p><div id="science-budget"></div><div id="science-plot"></div><details><summary>Resource observations, refinement evidence and method limitations</summary><pre id="science-metadata"></pre></details>`;
      const find = (id) => panel.querySelector("#" + id);
      find("science-inputs").textContent = state.physical ? JSON.stringify(state.physical, null, 2) : "No source definition adopted.";
      ...
      if (response) {
        ...
        if (response.result && r.association === "CURRENT") {
      ...
      find("science-connect").onclick = () => act(() => c.connect());
      find("science-adopt").onclick = () => act(() => c.adopt());
      for (const [id, method] of [["science-run", "start"], ["science-status", "status"], ["science-result", "result"], ["science-cancel", "cancel"]]) find(id).onclick = () => act(() => c[method]());
